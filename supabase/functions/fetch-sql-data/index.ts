...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const mockData = {
      ...
      por_cliente: [
        { cliente: 'TechCorp SA', valor: 45000 },
        { cliente: 'GlobalLogistics', valor: 38000 },
        { cliente: 'Alpha Industries', valor: 25000 },
        { cliente: 'Beta Retail', valor: 15000 },
        { cliente: 'Outros', valor: 6500 },
      ],
      por_tipo_documento: [
        { tipo: 'NFe', volume: 450 },
        { tipo: 'CTe', volume: 320 },
        { tipo: 'MDFe', volume: 150 },
        { tipo: 'NFSe', volume: 80 },
      ],
      por_status: [
        { status: 'Processado', volume: 650 },
        { status: 'Pendente', volume: 120 },
        { status: 'Erro', volume: 35 },
        { status: 'Cancelado', volume: 15 },
      ],
      last_update: new Date().toISOString(),
    }

    // 4. Update Cache
    const { error: upsertError } = await supabase.from('cache_dados_sql').upsert(
      {
        usuario_id,
        data: mockData,
