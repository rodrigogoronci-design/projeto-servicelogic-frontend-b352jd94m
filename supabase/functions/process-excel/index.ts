...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const jsonData = [
      { id: 1, cliente: 'Empresa Alpha', valor: 1500.0, vencimento: '2023-10-15', status: 'Pago' },
      {
        id: 2,
      ...
      },
      {
        ...
        valor: 4500.0,
        vencimento: '2023-10-18',
        status: 'Pendente',
      },
      { id: 5, cliente: 'Empresa Epsilon', valor: 120.0, vencimento: '2023-10-19', status: 'Pago' },
    ]
    ...
    const { error: insertError } = await supabase.from('dados_importados').insert({
      usuario_id,
    ...
    // Update log
    if (log_id) {
      await supabase.from('log_execucoes').update({ status: 'Concluído' }).eq('id', log_id)
    }
